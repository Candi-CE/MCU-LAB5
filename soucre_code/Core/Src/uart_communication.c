	...
	switch(index_buffer) {
		case 5: // user request command !RST#
			if (strcmp((const char *)buffer, userRequest) == 0) {
				...
				status_ACK = ERROR_G;
				ERROR_CODE_0 = CMD_NOT_FOUND;
			}
			break;
		case 4: // user done communicate with command !OK#
			if (strcmp((const char *)buffer, userOK) == 0) {
					if (status_ACK != WAITING_ACK) {
						ERROR_CODE_0 = REQUEST_DATA_FIRST;
						status_ACK = ERROR_G;
						break;
					}
					status_ACK = END_SEND;
